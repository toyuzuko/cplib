...
def _run_case(
    case: Case,
    splitmix_cls: Callable[[int], _Rng],
    xorshift_cls: Callable[[int], _Rng],
) -> list[object]:
...
def solve_case(case: Case) -> list[object]:
    return _run_case(case, SplitMix64, XorShift)


def check_primitives() -> None:
    seeds = (0, 1, 2, (1 << 64) - 1, 123456789)
    for seed in seeds:
        lib_sm = SplitMix64(seed)
        ref_sm = _SplitMix64Ref(seed)
        assert [lib_sm.random() for _ in range(8)] == [ref_sm.random() for _ in range(8)]

        lib_xs = XorShift(seed)
        ref_xs = _XorShiftRef(seed)
        assert [lib_xs.random() for _ in range(8)] == [ref_xs.random() for _ in range(8)]

        for width in (0, 1, 2, 7, 13, 32, 64):
            assert SplitMix64(seed).randbits(width) == _SplitMix64Ref(seed).randbits(width)
            assert XorShift(seed).randbits(width) == _XorShiftRef(seed).randbits(width)

        assert SplitMix64(seed).randrange(7) in range(7)
        assert XorShift(seed).randrange(3, 15) in range(3, 15)
        assert SplitMix64(seed).randrange(20, 2, -3) in range(20, 2, -3)
        assert XorShift(seed).randint(-4, 4) in range(-4, 5)

        assert SplitMix64(seed).choice([seed]) == seed
        assert XorShift(seed).choice([1, 2, 3]) in {1, 2, 3}

        values = [1, 2, 3, 4]
        XorShift(seed).shuffle(values)
